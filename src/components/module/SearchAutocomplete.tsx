...
export default function SearchAutocomplete({
  ...
  const [suggestions, setSuggestions] = useState<MuseumItem[]>([]);
  ...
  useEffect(() => {
    if (searchQuery.length > 1) {
      ...
      const filtered = allItems
        .filter((item) => {
          const titleMatch = searchTerms.every((term) => item.title.toLowerCase().includes(term));
  ...
  return (
    <div className="relative w-full md:w-[32rem] lg:w-[40rem]" ref={searchRef}>
      <div className="relative group">
        ...
        />
        <Search className="absolute left-3.5 top-1/2 w-5 h-5 text-gray-400 transition-colors transform -translate-y-1/2 group-hover:text-proj/70" aria-hidden="true" />
        {searchQuery && (
          <motion.button
            type="button"
          ...
          >
            {suggestions.map((item) => (
              <motion.button
                ...
                className="flex items-center p-3 w-full text-left transition-colors cursor-pointer hover:bg-gray-50"
                onClick={() => {
