...
const QrCodePage = (): React.JSX.Element | null => {
      ...
      >
        ...
        >
          ...
          <div className="w-full p-4 space-y-2 text-sm text-gray-600 bg-gray-100 rounded-lg">
            <p>
              <span className="font-medium">Booking Reference:</span> #{bookingDetails.bookingId || 'N/A'}
            </p>
            <p>
              <span className="font-medium">Name:</span> {bookingDetails.name || 'N/A'}
            </p>
            <p>
              <span className="font-medium">Date:</span> {formatDate(bookingDetails.date)}
            </p>
            <p>
              <span className="font-medium">Time Slot:</span> {bookingDetails.timeSlot || 'N/A'}
            </p>
            <p>
              <span className="font-medium">Guests:</span> {bookingDetails.attendees || 'N/A'}
            </p>
            <p>
              <span className="font-medium">Amenities:</span>{' '}
              {bookingDetails.amenities ? bookingDetails.amenities.join(', ') : 'None'}
            </p>
            <p>
              <span className="font-medium">Total:</span> ₹{bookingDetails.total || 'N/A'}
            </p>
          </div>
        </motion.div>
        <div className="flex flex-col gap-4 mt-8 sm:flex-row">
          ...
          <motion.button
            ...
            className="shine-button relative overflow-hidden flex items-center justify-center w-full px-6 py-3 text-sm font-medium text-white transition duration-300 bg-green-600 rounded-full hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            whileHover={{ scale: 1.03 }}
            whileTap={{ scale: 0.97 }}
          >
